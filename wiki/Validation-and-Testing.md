...
- **M-value sandbox chart interaction.** Verifies the ambient-equilibrium line <var>p</var><sub>t</sub> = <var>p</var><sub>amb</sub>, hover/focus legend highlighting for all four chart elements, legend placement directly below the graph, and identical typography for both pressure-axis titles and their `bar` units.
