...
**585 tests pass.** The Jest configuration in `package.json` is vestigial — `test:jest` and `test:watch` still work but are not the canonical runner; the CI gate is `npm test`, run on every pull request by `.github/workflows/ci.yml`.
...
- **Dive-plan runtime marker.** Verifies that both split dive-plan tables bind the shared runtime definition to their Runtime heading with a superscript asterisk, while practical and departure calculation details remain visible as separate unmarked notes.
