...
`results` is the object returned by `calculateTissueLoading` (see [[Algo-01-Ascent-Simulation]]).
...
- [[Algo-01-Ascent-Simulation]] — produces the `results` object consumed here.
- [[Algo-03-First-Stop-Ramped-GF]] — the `findGFLowAnchor` function used for fallback anchor computation.
- [[Algo-04-Deco-Stop-Loop]] — `generateDecoSchedule` returns the `pAnchor` to pass in as `providedPAnchor`.
- [[Model-05-Gradient-Factors]] — `interpolateGF` and the ramp geometry.
