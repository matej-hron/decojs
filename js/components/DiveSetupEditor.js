...
function getGasDisplayName(gas) {
    return gas.name === 'Air'
        ? translate('diveEditor.gases.airShort', 'Air')
        : gas.name;
}
...
function formatGasMod(
    ...
    const mod14 = calculateMOD(
        o2Fraction, 1.4, surfacePressure, pressurePerMeter
    );
    if (isBottomGas) {
        return fmt(translate('diveEditor.modBottom', 'MOD: {0}\u00a0m'), mod14);
    }

    const mod16 = calculateMOD(
        o2Fraction, 1.6, surfacePressure, pressurePerMeter
    );
    const switchDepth = Math.max(0, Math.floor(mod16 / 3) * 3);
    return fmt(
        translate('diveEditor.mod', 'MOD: {0}\u00a0m\ndeco MOD: {1}\u00a0m\nrecommended switch: {2}\u00a0m'),
        mod14,
        mod16,
        switchDepth
    );
...
const DEFAULT_EDITOR_OPTIONS = {
    ...
    autoGenerateDelay: 250,
    alignRuntimeDepartures: false,
    showSafetyStop: true,
...
export class DiveSetupEditor extends EventTarget {
    ...
    constructor(container, config = {}) {
        ...
        this._autoGenerateTimer = null;
        this._isGeneratingProfile = false;
     ...
     */
    destroy() {
        if (this._autoGenerateTimer !== null) {
            clearTimeout(this._autoGenerateTimer);
            this._autoGenerateTimer = null;
        }
    ...
    _buildQuickSetup() {
        ...
        section.innerHTML = `
            ...
            <div class="dse-quick-inputs">
                ${this.options.showGenerateButton ? `<p class="dse-hint">${
                    this.options.autoGenerateProfile
                        ? translate('diveEditor.quickSetup.autoHint', 'The profile updates automatically after each change.')
                        : translate('diveEditor.quickSetup.hint', 'Set depth and bottom time, then click Generate below.')
                }</p>` : ''}
                <div class="dse-row">
                    <div class="dse-field">
                ...
                ${this.options.showGenerateButton && !this.options.autoGenerateProfile
                    ? `<button class="dse-generate-btn btn btn-primary">🔄 ${translate('diveEditor.generateProfile', 'Generate Profile')}</button>`
                    : ''}
                ${this.options.showGenerateButton || this.options.autoGenerateProfile ? `
        ...
        const handleQuickInput = () => {
            ...
            this._scheduleAutoGenerateProfile();
        };
        this.elements.quickDepth.addEventListener('input', handleQuickInput);
        this.elements.quickTime.addEventListener('input', handleQuickInput);
        ...
        if (this.elements.safetyStopEnabled) {
            ...
            this.elements.safetyStopDepth.addEventListener('input', handleQuickInput);
            this.elements.safetyStopTime.addEventListener('input', handleQuickInput);
        }
    ...
    _buildGradientFactors() {
        ...
        section.open = false; // Collapsed by default
        section.innerHTML = `
            <summary>🎚️ ${translate('diveEditor.gf.title', 'Decompression Model — Bühlmann')} <span class="dse-summary-hint">(GF 100/100)</span></summary>
            <div class="dse-gf-content">
                <div class="dse-algorithm-row">
                    ...
                    <select class="dse-algorithm-select form-input">
                        <option value="ZH-L16A">Bühlmann ZH-L16A (${translate('diveEditor.gf.algorithmExperimental', 'experimental')})</option>
                        <option value="ZH-L16B">Bühlmann ZH-L16B (${translate('diveEditor.gf.algorithmTables', 'tables')})</option>
                        <option value="ZH-L16C">Bühlmann ZH-L16C (${translate('diveEditor.gf.algorithmComputers', 'computers')})</option>
                    </select>
                </div>
                <h4 class="dse-gf-settings-title">${translate('diveEditor.gf.settingsTitle', 'Gradient Factor Settings')}</h4>
    ...
    _getWaterType() {
        return getSelectedWaterType(this.elements);
    }

    _getPressurePerMeter() {
        return getPressurePerMeter({
            waterType: getSelectedWaterType(this.elements)
        });
    }
    ...
    _updateEnvironmentDisplay() {
        ...
        const waterType = getSelectedWaterType(this.elements);
        const waterLabel = {
            [WATER_TYPES.STANDARD]: translate(
                'diveEditor.environment.waterStandardShort',
                'EN'
            ),
            [WATER_TYPES.FRESH]: translate(
                'diveEditor.environment.waterFreshShort',
                'fresh'
            ),
            [WATER_TYPES.SEA]: translate(
                'diveEditor.environment.waterSeaShort',
                'sea'
            )
        }[waterType];
        if (this.elements.environmentSummaryHint) {
            this.elements.environmentSummaryHint.textContent =
                `(${fmtNum(altitude, 0)}\u00a0m · ${waterLabel})`;
        }
    ...
    _createGasCard(gas, index) {
            ...
            <div class="dse-gas-content">
                <div class="dse-gas-row">
                    ...
                    <select class="dse-gas-preset form-select">
                        ${gasOptions.map(g =>
                            `<option value="${escHtml(g.id)}" ${matchingPreset?.id === g.id ? 'selected' : ''}>${escHtml(getGasDisplayName(g))}</option>`
                        ).join('')}
                ...
                <div class="dse-gas-mod">
                    <span class="dse-hint">${formatGasMod(
                        gas.o2,
                        modSurfacePressure,
                        getPressurePerMeter({
                            waterType: getSelectedWaterType(this.elements)
        ...
        presetSelect.addEventListener('change', () => {
            if (presetSelect.value === 'custom') {
                ...
                if (preset) {
                    ...
                    this._updateGasModDisplay(modDisplay, preset.o2, isBottomGas);
                    this._updateWaypointGasDropdowns();
                    this._onInputChange();
                    this._updateNDLDisplay();
                }
        ...
        const updateCustomGas = () => {
            ...
            this._updateGasModDisplay(modDisplay, o2, isBottomGas);
            this._updateWaypointGasDropdowns();
            this._onInputChange();
            this._updateNDLDisplay();
        };
    ...
    _updateGasModDisplay(modDisplay, o2Fraction, isBottomGas) {
        const modSurfacePressure = this._getModSurfacePressure?.() ?? 1;
        modDisplay.textContent = formatGasMod(
            o2Fraction,
            modSurfacePressure,
            getPressurePerMeter({
                waterType: getSelectedWaterType(this.elements)
        ...
        const updateDropdowns = (body) => {
            ...
            body.querySelectorAll('.dse-wp-gas').forEach(select => {
                ...
                select.innerHTML = this.currentGases.map(gas => 
                    `<option value="${escHtml(gas.id)}" ${gas.id === currentValue ? 'selected' : ''}>${escHtml(getGasDisplayName(gas))}</option>`
                ).join('');
            });
        };
    ...
    _updateDivePlan(tbody) {
        ...
        planDiv.innerHTML = renderDivePlanTableHTML(waypoints, this.currentGases, {
            ...
            decoSacRate,
            reserve,
            surfacePressure: this._getSurfacePressure(),
            pressurePerMeter: this._getPressurePerMeter()
        });
    }
    ...
    _addWaypointRow(tbody, time = '', depth = '', gasId = '') {
        ...
        const gasOptions = this.currentGases.map(gas => 
            `<option value="${escHtml(gas.id)}" ${gas.id === gasId ? 'selected' : ''}>${escHtml(getGasDisplayName(gas))}</option>`
        ).join('');
    ...
    _insertWaypointAfter(afterRow, tbody) {
        ...
        const gasOptions = this.currentGases.map(gas => 
            `<option value="${escHtml(gas.id)}" ${gas.id === currentGasId ? 'selected' : ''}>${escHtml(getGasDisplayName(gas))}</option>`
        ).join('');
    ...
    _updateNDLDisplay() {
        ...
        const { ndl, ndlExact } = getNDLForDepth(
            maxDepth,
            gas,
            gfHigh,
            surfacePressure,
            this._getPressurePerMeter()
        );
        ...
        if (status.state === 'deco') {
            ...
            try {
                const result = generateDecoProfile(
                    ...
                    {
                        decoMode: decoModeNDL,
                        gasSwitchTime: gasSwitchTimeNDL,
                        surfacePressure,
                        alignRuntimeDepartures: this.options.alignRuntimeDepartures
                    }
                );
    ...
    _generateProfile() {
        ...
        const surfacePressure = this._getSurfacePressure();
        this._isGeneratingProfile = true;
        try {
            let result;
            try {
    ...
    _onInputChange(preserveDecisionAudit = false) {
        ...
        if (this.options.autoGenerateProfile && !this._isGeneratingProfile) {
            this._scheduleAutoGenerateProfile();
            return;
        }
    ...
    _scheduleAutoGenerateProfile() {
        if (!this.options.autoGenerateProfile || this._isGeneratingProfile) return;

        if (this._autoGenerateTimer !== null) {
            clearTimeout(this._autoGenerateTimer);
        }
        this._autoGenerateTimer = setTimeout(() => {
            this._autoGenerateTimer = null;
            this._generateProfile();
        }, this.options.autoGenerateDelay);
    }

    _updateSummaryHints() {
        // Update Gases summary hint
        if (this.elements.gasesSummaryHint) {
            const gasNames = this.currentGases.map(getGasDisplayName).join(' + ');
